.PHONY: up down logs ps build test seed migrate deploy deploy-prod
...
# Production deploy on the VM: starts the isolated Postgres/Redis stack,
# applies migrations, and starts all backend services.
deploy:
